...
import os #functions that interact with OS
from prefetch_generator import BackgroundGenerator #fetches data in background during training for better performance
from ptflops import get_model_complexity_info #model complexity information
# from models.network import GCNSegNet as seg_model
from unet.unet_model import UNet as unet
...
import numpy as np
import random
from tensorboardX import SummaryWriter #writes tensorboard logs for visualization during training
import torch
# from eval_mo import validate, sigmoid_output_dice #model evaluation
...
#Creates directories for saving checkpoints and tensorboard logs
exp_name = 'MedicalSeg'

#creates previous sava path by joining an empty string, experiment name and empty string
#checkpoints stored
previous_save_path = os.path.join('', exp_name, '')

#current checkpoints will be saved
...
#Directories used to save checkpoints and tensorboard logs
check_mkdir(save_path)
check_mkdir(runs_path)

#Writes tensorboard logs
#Visualizes metrics (loss&accuracy)
...
def main():
    ...
    parser.add_argument('--np_ratio', type=float, default=0.005)#Specifies the ratio of negative pixels in the dataset
    parser.add_argument('--k_ratio', type=float, default=0.5)#Specifies the ratio for kernel size
    parser.add_argument('--os', type=int, default=8)

    parser.add_argument('--eps', type=float, default=1e-3)# Specifies the epsilon value for numerical stability
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--opt', type=str, default='adam', choices=('sgd', 'adam'))
    parser.add_argument('--resume', default='') #Specifies the checkpoint file to resume training from
    ...
    #variable is used by CUDA to specify which GPU devices are visible to the program.
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    #indicates the GPU device(s) selected for training.
    print(args.gpu)
    #random seed for PyTorch's random number generator
    torch.manual_seed(args.seed)
    random.seed(args.seed)
    np.random.seed(args.seed)
    #executes if CUDA is available
    if args.cuda:
    ...
    #checks if no resume checkpoint file is specified
    if len(args.resume) == 0:
        #training starts from first epoch
        curr_epoch = 1
        args.best_record = {'epoch': 0, 'valid_loss': 1, 'dice_value': 0}
    else:
        print('training resumes from' + args.resume)
        #loads directory of model. loads checkpoint file
